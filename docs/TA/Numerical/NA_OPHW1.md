...
**3．上溢值(overflow)** 是计算机能表示的最大浮点数．上溢值乘以 2 之后，在计算机中会保存为 ```+Inf```（有些可能保存为```NaN```）．所以为了寻找上溢值，只需从 1 出发，不断乘以 2，即可得到上溢值．
...
在本题中，第一组初值满足 $3\varepsilon_1-\varepsilon_0\ne 0$，而第二组初值满足 $3\varepsilon_1-\varepsilon_0 = 0$．如果采用方式一，后续得到的数都是整数，所以没有舍入误差；如果采用方式二，后续得到的数会出现精度丢失，这是因为计算机中无法精确存储 $\dfrac{13}{3}$ 与 $\dfrac{4}{3}$，计算过程会带来误差．

因此，这个算法的稳定性与所给初值以及运算顺序都有关．

{: .remark}
> 有时候即使绝对误差很大，但是相对误差一直保持在一个很小的范围，我们可以认定这个算法是稳定的．

**加分点：** 探究了对于初值 $x_0=1$，$x_1=4$，采用方式一和方式二对结果带来的影响．
