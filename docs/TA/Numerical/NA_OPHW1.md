...
所以为了寻找上溢值，只需从 1 出发，不断乘以 2，即可得到 $x=(1000\cdots 0)_2$ ，这个数再乘以 2 就大于上溢值，会得到 ```+Inf```，记最高位是第 $k$ 位，则 $x=2^{k-1}$，然后再从最高位出发往回不断除以 2，得到求和 $\sum\limits_{i=0}^{k-1}fl(\dfrac{x}{2^i})$，即为上溢值 $(1111\cdots 100\cdots 0)_2$．
