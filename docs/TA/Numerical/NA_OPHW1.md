...
**3．上溢值(overflow)** 是计算机能表示的最大浮点数．上溢值加上 1 之后，在计算机中会保存为 ```+Inf```（有些可能保存为```NaN```）．所以为了寻找上溢值，只需从 1 出发，不断乘以 2，即可得到 $x=(1000\cdots 0)_2$ ，这个数再乘以 2 就大于上溢值，会得到 ```+Inf```，记最高位是第 $k$ 位，则 $x=2^{k-1}$，然后再从最高位出发往回不断除以 2，得到求和 $\sum\limits_{i=0}^{k-1}\dfrac{x}{2^i}$，即为上溢值 $(1111\cdots 1)_2$．
