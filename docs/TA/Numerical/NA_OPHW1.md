...
$$0.1001100110011001100110011001100110011001100110011001 \times 10^{-2}.$$
...
- 没有验证精度丢失最多一位，或者只是截断到 $\dfrac{x^3}{3!}-\dfrac{x^5}{5!}$ 就断言得到的结果与 $x-\sin x$ 的真实值接近．然而如果这样截断，从截断误差的角度来看根本就不接近！
...
（2）用第一组初值求出了$\lbrace x_n\rbrace$的通项公式 $x_n=(\dfrac{1}{3})^n$，然后就说数列递推式中的减法出现了“相减相消”现象．但是$\lbrace x_n\rbrace$并不是一直都很小的，迭代到 $n=15$ 时 $x_n$ 是一个比较大的数，就不再是相减相消了．也就是相当于没有揭示算法不稳定的根本原因．合理的解释是用舍入误差来做分析．

（3）用条件数来分析，但是该同学先导出 $\lbrace x_n\rbrace$ 的通项公式

$$f_n(x)=\dfrac{12-3x}{11}\times(\frac{1}{3})^n + \dfrac{3x-1}{n}\times 4^n$$

其中 $x_0=1$，$x_1=x$，然后分析了 $f_n(x)$ 的条件数．这样的做法看似合理，但是本题并不是直接计算 $f_n(x)$，而是计算一个数列递推式，所以我们哟呵分析这个数列递推式，而不是要分析 $f_n(x)$．
