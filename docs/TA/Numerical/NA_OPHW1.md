...
**3．上溢值(overflow)** 是计算机能表示的最大浮点数．上溢值乘以 2 之后，在计算机中会保存为 ```+Inf```（有些可能保存为```NaN```）．所以为了寻找上溢值，只需从 1 出发，不断乘以 2，即可得到最高位 $(1000\cdots 0)_2$，然后再从最高位出发往回不断除以 2，最终得到上溢值 $(1111\cdots 1)_2$．
